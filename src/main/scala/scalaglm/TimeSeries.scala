...
/** Utilities for multivariate time series
  *
  * These utilities assume that the time series is stored in a Breeze `DMD` with
...
object TimeSeries {
  ...
  def autocovariance(x: DMD, lag: Int, centre: Boolean = true): DMD = {
    if (lag < 0)
      ...
      val xMat = if (centre) meanCentre(x) else x
      covariance(
        xMat(0 until (x.rows - lag), ::),
        xMat(lag until x.rows, ::).copy,
        centre = false
      )
    }
  }
  ...
  /** Fit a mean-zero VAR(p) model to a time series via least squares. Since the
    * model being fit is mean zero, you may want to mean-centre your data.
    *
    * @param x
    *   Time series
    * @param p
    ...
    *
    * @return
    *   The list of `p` fitted VAR matrices, together with the lower Cholesky
    *   triangle of the estimated innovation variance matrix
    */
  def fitVar(x: DMD, p: Int, centre: Boolean = true): (List[DMD], DMD) = {
    ...
    val xMat = if (centre) meanCentre(x) else x
    val size = n - p
    val bigX = (0 until p).toList
      .map(i => xMat((p - i - 1) until (p - i - 1 + size), ::))
    ...
    val phiL =
      (0 until p).toList.map(i => phi((i * m) until ((i + 1) * m), ::).copy.t)
    val e = xp - q * qtx
    val r2 = qr.justR(e)
  ...
  def isStat(phi: List[DMD]): Boolean = {
    ...
    val v1phi = DenseMatrix.vertcat(
      phiConc,
      DenseMatrix.horzcat(
        DenseMatrix.eye[Double](m * (p - 1)),
